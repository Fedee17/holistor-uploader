...
async function loadClientes(forzar = false) {
  ...
  try {
    const res = await fetch(WEBHOOK.clientes());
...
async function resetProceso() {
  ...
  try {
    ...
    const res = await fetch(WEBHOOK.reset(), {
      method: 'POST',
...
async function guardarRegistroEnN8n(zone, registros, receptorCuit) {
  ...
  try {
    ...
    let resReg;
    try {
      resReg = await fetch(WEBHOOK.registro(), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function iniciarPolling() {
  ...
  pollingInterval = setInterval(async () => {
    try {
      const cuit = clienteSeleccionado?.cuit || '';
      const per = periodo ? `${periodo.anio}-${String(periodo.mes).padStart(2,'0')}` : '';
      const res = await fetch(`${WEBHOOK.estado()}?cliente_cuit=${encodeURIComponent(cuit)}&periodo=${encodeURIComponent(per)}`);
      if (!res.ok) return;
      const data = await res.json();
      if (!data) return;
...
async function reintentar() {
  ...
  try {
    ...
    if (data.fallidos && data.fallidos.length > 0) {
      ...
      showAlert(`Aún hay ${data.fallidos.length} ${pluralizar(data.fallidos.length, 'error', 'errores')}.`, 'error');
    } else {
      showAlert('Reintento completado.', 'success');
    }
  } catch(e) {
...
function renderQueue() {
  ...
  if (pending > 0) {
    ...
    document.getElementById('loteInfoText').textContent = `${lotes} ${pluralizar(lotes, 'lote', 'lotes')} de 50 facturas`;
  } else {
...
async function verificarDuplicado(nombre) {
  try {
    const cuit = clienteSeleccionado?.cuit || '';
    const res = await fetch(WEBHOOK.verificar(), {
      method:'POST',
      headers: { 'Content-Type': 'application/json' },
